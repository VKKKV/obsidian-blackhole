...
const {normalizeSettings,DEFAULT_SETTINGS,runtimeRenderScale}=loadTS('src/config.ts');
...
test('settings reject invalid types and preserve stored preference separate from budget',()=>{
 ...
 assert.equal(s.renderScale,1);assert.equal(runtimeRenderScale(s),.35);assert.equal(s.nSteps,64);
...
test('metrics count mixed CJK and words and cache unchanged editor content',()=>{
 ...
 cache.level(DEFAULT_SETTINGS,0);cache.level(DEFAULT_SETTINGS,10000);assert.equal(reads,1);
 cache.invalidate('word-count');cache.level(DEFAULT_SETTINGS,10001);assert.equal(reads,2);
 cache.invalidate('word-count');cache.level(DEFAULT_SETTINGS,10002);assert.equal(reads,2);
});
