...
export interface ShaderParams {
  ...
  breakMin: number;
  idleFadeSec: number;
  tokenGlideMin: number;
  tokenGlideMax: number;
  tokenGlideRate: number;
}

/**
 * Build the fragment shader source with tunable consts baked in.
 *
 * All values that the user can tweak in settings are injected as compile-time
